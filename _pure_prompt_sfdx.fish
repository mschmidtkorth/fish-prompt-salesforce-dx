...
function _pure_prompt_sfdx \
    ...
    if test -e $configDir/sfdx-project.json # Is current dir a SFDX project (sub)directory?
        ...
        if test -n $theme_powerline_fonts # TODO: To implement arrow segmentation (see Jochen's example)
            ...
            # TODO: MesloLGS NF (Nerd Font) lets the unicode characters appear tiny (eg Cloud)
        end
        echo $sfdx_prompt
    end
end
